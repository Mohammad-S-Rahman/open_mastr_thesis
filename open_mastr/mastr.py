...
class Mastr:
    ...
    #   validate_parameter_format_for_mastr_init(engine)
        if engine == 'sqlite':
            ...
            engine = create_engine("postgresql://tester:1234@localhost/BulkTest") # how to save the credentials? Users don't wanna do this again n again
            os.makedirs(self._sqlite_folder_path, exist_ok=True)
    ...
    def download(
        self,
        method="bulk", #Change here if we wish to do a Bulk or API download. Bulk should be written in small letters. API in capital
        data=None,
        date=None,
        bulk_cleansing=True,
        api_processes=None,
